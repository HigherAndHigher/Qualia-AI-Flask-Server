...
def create_animation():
    ...
    response = requests.post(
        "https://api.d-id.com/animations", json=data, headers=headers
    )
...
def get_animation(animation_id):
    ...
    reponse = requests.get(
        "https://api.d-id.com/animations/" + animation_id, headers=headers
    )
...
def create_ai_friend():
    ...
    ai_friend_data = {
        ...
        "favorite_and_disliked_foods_and_drinks": data.get(
            "favorite_and_disliked_foods_and_drinks"
        ),
        "spicy_or_sweet_food_preference": data.get("spicy_or_sweet_food_preference"),
        "cooking_skills_and_signature_dishes": data.get(
            "cooking_skills_and_signature_dishes"
        ),
...
def update_ai_friend():
    ...
    db.collection("ai_friends").document(id).update(ai_friend_data)
...
def user_info():
    ...
    db.collection("ai_friends").document(friend_id).collection(
        "user_sessions"
    ).document("current_user").set(user_data)
...
def chat_with_ai_friend():
    ...
    chat_history = [
        {"role": doc.to_dict()["role"], "message": doc.to_dict()["message"]}
    ...
    chat_ref = doc_ref.collection("chat_history").document()
    chat_ref.set(
        {"role": "bot", "message": response, "timestamp": firestore.SERVER_TIMESTAMP}
    )
...
def generate_response(ai_friend_data, user_data, user_message, chat_history):
    ...
    if (
        response.startswith("AIのメッセージ")
        or response.startswith("AIの応答")
        or response.startswith("AI自分の名前")
...
def convertToGif(video_path):
    try:
        clip = VideoFileClip(video_path)
        gif_filename = os.path.join(
            GIF_FOLDER, os.path.splitext(os.path.basename(video_path))[0] + ".gif"
        )
...
def convert_video():
    try:
        ...
        gif_filename = convertToGif(video_path)
        if not gif_filename:
            return jsonify({"error": "Conversion failed"}), 500

        return send_file(gif_filename, as_attachment=True)
